...
<plugin name="fix-focus" version="0.1"
    ...
    <item>
        ...
        <description>
            <p>
                A key when may be pressed to wrest focus from plugins.
...
for (let [k, v] in Iterator({
    ...
    CONTEXT[k] = ctypes.StructType(k, [toObject([[prop, type]]) for ([prop, type] in Iterator(v))]);
...
function declare(lib, obj) {
    for (let [name, proto] in Iterator(obj))
        CONTEXT[name] = lib.declare.apply(lib, [name, ctypes.default_abi].concat(proto));
}
...
function findXID(win) {
    ...
    let title = win.document.title, mytitle = "{dactyl-window:" + Date.now() + "}";;
    try {
        for (let i in util.range(1, 1000)) {
            win.document.title = mytitle;
            util.threadYield(true);
            for (let [i, window] in iter(list)) {
                let prop = getProperty(window, _NET_WM_NAME, UTF8_STRING, 0, 1024);
                try {
...
if (isWorker) {
    ...
    function onmessage(event) {
        ...
        unptr(event.data.done, ctypes.int.ptr.ptr).contents.value = done.address();

        let ev;
        while(!done.value && (ev = xcb_wait_for_event(connection)) && !ev.isNull()) {
            try {
                if (!ev.isNull()) {
                    // dump(ev.contents);
    ...
    let win = findXID(window);
    util.dump("Got window: " + win);
    ...
    function ptr(val) String(ctypes.cast(val, ctypes.int64_t).value);
    done = ctypes.int.ptr();

    var worker = ChromeWorker("file://" + PATH);
